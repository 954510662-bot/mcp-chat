...
import { MultiAgentPlatform } from 'mcp-chat-core';

let platform: MultiAgentPlatform | null = null;
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // 初始化平台
  platform = new MultiAgentPlatform();
  await platform.init();

  // 注册命令
  context.subscriptions.push(
    vscode.commands.registerCommand('mcp-chat.start', () => {
      createOrShowPanel(context);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('mcp-chat.newTask', () => {
      if (panel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('mcp-chat.clearHistory', () => {
      // 清空历史
    })
  );
...
class SidebarViewProvider implements vscode.WebviewViewProvider {
  ...
  private async _executeTask(prompt: string) {
    if (!platform) { return; }
...
function getChatPanelHtml(webview: vscode.Webview, context: vscode.ExtensionContext) {
  return `
    <!DOCTYPE html>
    <html lang="zh-CN">
    <head>
